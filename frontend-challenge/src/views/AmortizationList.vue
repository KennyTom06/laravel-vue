    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      ...
      <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center">
          <h1 class="text-3xl font-bold text-gray-900">Amortizations</h1>
      ...
      <!-- Pay Form -->
      <div class="bg-white shadow rounded-lg p-6 mb-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Pay Amortizations</h3>
      ...
      <div class="bg-white shadow overflow-hidden sm:rounded-md">
        <ul class="divide-y divide-gray-200">
          <li v-for="amortization in amortizations" :key="amortization.id">
            <router-link
              :to="`/amortization/${amortization.id}`"
                  ...
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      ...
                      <div class="text-sm text-gray-500">
                        Project: {{ amortization.project?.name || 'N/A' }} | Promoter: {{ amortization.promoter?.name || 'N/A' }}
                      </div>
                    </div>
                  </div>
...
const amortizations = computed(() => store.getters.amortizations)
const pagination = computed(() => store.getters.pagination)
const loading = computed(() => store.getters.loading)
const error = computed(() => store.getters.error)

const fetchAmortizations = () => {
  store.dispatch('fetchAmortizations', filters.value)
}

const changePage = (page: number) => {
  if (page >= 1 && page <= pagination.value?.last_page) {
    store.dispatch('fetchAmortizations', { ...filters.value, page })
  }
}
...
const payAmortizations = async () => {
  try {
    const date = payDate.value || null
    await store.dispatch('payAmortizations', date)
